...
export default function App() {
  ...
  // Состояния для двойного слайдера
  const [rangeMin, setRangeMin] = useState(5);
  const [rangeMax, setRangeMax] = useState(10);
  
  // Состояние для координат (Широта и Долгота)
  ...
  // Логика обновления координат при движении мыши по контенту
  const handleMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
    const rect = e.currentTarget.getBoundingClientRect();
    const x = e.clientX - rect.left;
    const y = e.clientY - rect.top;
    
    // Имитация пересчета пикселей в градусы
    const lat = 60.5000 - (y / rect.height) * 0.5;
    const lng = 30.1000 + (x / rect.width) * 0.8;
    ...
    <div className="app">
      ...
      <div className="top-bar">
        ...
        <div className="top-bar-right">
          <div className="theme-toggle"><span><img src={themeIcon} className="bottom-icon" alt="" /></span></div>
          <div className="user-profile">
            <span className="user-name">e_golovach|admin</span>
          </div>
          <button className="logout-btn"><span><img src={exitIcon} className="bottom-icon" alt="" /></span></button>
        </div>
      </div>
  ...
  <div className="sidebar-label-row">
    ...
    <button className="refresh-btn-active" ><img src={refreshIcon} className="sidebar-icon-img" alt="" /></button>
  </div>
  <select className="ui-element sidebar-select">
    <option>СЕВ-ЗАП Лесничество (2)</option>
    <option>УФО Кыштымский карлик</option>
  </select>
</div>
  ...
  <div className="double-range-slider">
    ...
    {/* НОВЫЙ ЭЛЕМЕНТ: Цветная заполнитель между ползунками */}
    <div 
  ...
  <button className="refresh-btn-active" ><img src={adddetIcon} className="sidebar-icon-img" alt="" /></button>
</div>

          <div className="sidebar-scroll">
            {items.map((item, i) => (
              <div key={i} className="scroll-item">{item}. <span className="bottom-label">19.04.2026 46.23*67.52</span></div> 
            ))}
          </div>
        </aside>

        {/* ОСНОВНОЙ КОНТЕНТ С ПОЛОСКОЙ КООРДИНАТ */}
    ...
    <button className="zoom-btn">−</button>
      ...
      {/* BOTTOM BAR */}
        ...
        <div className="bottom-item">
          <span className="bottom-icon"><img src={distanceIcon} className="bottom-icon" /></span>
          <span className="bottom-label">Расстояние</span>
        </div>
        <div className="bottom-item">
                    <span className="bottom-icon"><img src={addIcon} className="bottom-icon" /></span>
          <span className="bottom-label">Добавить</span>
        </div>
        <div className="bottom-item">
                    <span className="bottom-icon"><img src={layersIcon} className="bottom-icon" /></span>
          <span className="bottom-label">Слои</span>
        </div>
        <div className="bottom-item">
        <span className="bottom-icon"><img src={shareIcon} className="bottom-icon" /></span>
          <span className="bottom-label">Поделиться</span>
        </div>
        <div className="bottom-item">
        <span className="bottom-icon"><img src={screenshotIcon} className="bottom-icon" /></span>
          <span className="bottom-label">Снимок</span>
        </div>
        <div className="bottom-item">
      <span className="bottom-icon"><img src={tutorIcon} className="bottom-icon" /></span>
